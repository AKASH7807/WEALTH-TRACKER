...
const MonthlyFinanceInsight = () => {
  ...
  const handleNumberInput = useCallback((value, setter) => {
    setter(value.replace(/[^\d.]/g, "").replace(/(\..*?)\..*/g, "$1"));
  }, []);

  const incomeNum = Number(income);
  const expensesNum = Number(expenses);
  const isValid = incomeNum > 0 && expensesNum >= 0;
  ...
  const calculateReport = useCallback(() => {
    if (
      ...
      Number.isNaN(expensesNum)
    ) {
      toast.error("Please enter valid income and expenses.");
      return;
    }

    const ratio = Math.min((expensesNum / incomeNum) * 100, 100);
  ...
  const progressBarStyle = useMemo(
    () => ({
      width: report ? `${Math.min(report.ratio, 100)}%` : "0%",
    ...
    <div className="w-full max-w-md mx-auto bg-white p-6 rounded-2xl shadow-lg border border-gray-100">
      ...
      <div className="space-y-4">
        <NumericInput
          ...
          value={income}
          onChange={(val) => handleNumberInput(val, setIncome)}
        />
        <NumericInput
          label="Monthly Expenses"
          value={expenses}
          onChange={(val) => handleNumberInput(val, setExpenses)}
        />
      </div>
      ...
      <button
        ...
        disabled={!isValid}
        className={`w-full mt-6 py-2.5 rounded-lg text-white font-medium transition ${
          isValid
            ? "bg-gradient-to-r from-purple-600 to-purple-500 hover:opacity-95 shadow-md"
      ...
      {report && (
        <div className="mt-6 space-y-5">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <SummaryCard
              title="Income"
              value={currencyFormatter.format(report.income)}
            />
            <SummaryCard
              title="Expenses"
          ...
          <p className="text-right text-xs mt-1 text-gray-500">
            Expense Ratio: {report.ratio}%
          </p>

          <div
            className="p-4 rounded-xl border-l-4 bg-gray-50 text-sm sm:text-base"
